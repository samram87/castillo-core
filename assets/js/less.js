...
Meprototype = new Directive();
Meprototype.type = "Media";
Meprototype.isRulesetLike = true;
Meprototype.accept = function (visitor) {
    if (this.features) {
        this.features = visitor.visit(this.features);
    }
    if (this.rules) {
...
Meprototype.genCSS = function (context, output) {
    output.add('@media ', this.currentFileInfo, this.index);
    this.features.genCSS(context, output);
    this.outputRuleset(context, output, this.rules);
};
Meprototype.eval = function (context) {
    ...
    try {
        mefeatures = this.features.eval(context);
    }
    finally {
        if (strictMathBypass) {
    ...
    merules = [this.rules[0].eval(context)];
    context.frames.shift();

    context.mediaPath.pop();

    return context.mediaPath.length === 0 ? meevalTop(context) :
                meevalNested(context);
};
Meprototype.evalTop = function (context) {
    var result = this;
...
Meprototype.evalNested = function (context) {
    var i, value,
...
Meprototype.permute = function (arr) {
    if (arr.length === 0) {
        return [];
...
Meprototype.bubbleSelectors = function (selectors) {
    if (!selectors) {
        return;
    }
